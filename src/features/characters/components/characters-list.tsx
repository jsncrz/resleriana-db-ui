    ...
    <div className="grid grid-cols-3 sm:grid-cols-6 gap-8 items-center">
      {characters.map((chara) => {
        const attributeVariants = {
          ...
          slash: 'from-slash-dark/25 to-slash-light/25 hover:shadow-slash-light/20',
        ...
        return (
          <div
            ...
            key={chara.id}
            className={`cursor-pointer max-w-md rounded-3xl p-5px transition-[scale,box-shadow,border-width] duration-300 
            ease-in-out hover:scale-105 hover:shadow-lg hover:border-4 border-3 border-foreground bg-gradient-to-b ${attributeVariants[attribute]}`}
          >
            <div
              className={`rounded-[calc(1.5rem-1px)] p-5 items-center text-center`}
            >
              <img
                className="size-full border-2 border-foreground rounded-xl"
                src={imgUrl + chara.id + '_icon.png'}
              ></img>
              <span className="text-lg font-bold antialiased text-shadow-md text-shadow-background/50">{chara.name}</span>
              <br />
              <span className="text-md antialiased text-shadow-sm text-shadow-background/50">{chara.anotherName}</span>
            </div>
          </div>
        );
      })}
